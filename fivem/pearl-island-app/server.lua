...
-- FiveM server 只需 clone fivem-release branch，pi_update 自動 git pull 更新
RegisterCommand('pi_update', function(source)
    ...
    local ok = os.execute('cd "' .. RESOURCE_DIR .. '" && git pull')
    if ok ~= 0 then
        print('[pearl-island] git pull 失敗，請確認 git 已安裝且此資料夾是 git repo')
        return
    end
