...
async function compressVideos() {
    console.log('Videos: compressing to 1280x720 H.264 CRF23...')
    ...
    for (const file of files) {
        ...
        execSync(
            `ffmpeg -y -loglevel error -i "${src}" -vf "scale=1280:720:force_original_aspect_ratio=decrease,scale=trunc(iw/2)*2:trunc(ih/2)*2" -c:v libx264 -crf 23 -preset medium -c:a aac -b:a 128k "${out}"`,
            { stdio: 'inherit' }
        )
        console.log(`  ok: ${file}`)
    }
